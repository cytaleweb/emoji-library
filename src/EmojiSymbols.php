<?php
namespace iksaku\Emoji;

/**
 * Class EmojiSymbols
 * @package iksaku\Emoji
 */
class EmojiSymbols extends EmojiSection {
    const HEART = "\u{2764}";
    const RED_HEART = self::HEART;

    const YELLOW_HEART = "\u{1F49B}";


    const GREEN_HEART = "\u{1F49A}";

    const BLUE_HEART = "\u{1F499}";

    const PURPLE_HEART = "\u{1F49C}";

    const BLACK_HEART = "\u{1F5A4}";

    const BROKEN_HEART = "\u{1F494}";

    const HEART_EXCLAMATION = "\u{2763}";
    const HEAVY_HEART_EXCLAMATION = self::HEART_EXCLAMATION;

    const TWO_HEARTS = "\u{1F495}";
    const REVOLVING_HEARTS = "\u{1F49E}";

    const HEARTBEAT = "\u{1F493}";
    const BEATING_HEART = self::HEARTBEAT;

    const HEARTPULSE = "\u{1F497}";
    const GROWING_HEART = self::HEARTPULSE;

    const SPARKLING_HEART = "\u{1F496}";

    const CUPID = "\u{1F498}";
    const HEART_WITH_ARROW = self::CUPID;

    const GIFT_HEART = "\u{1F49D}";
    const HEART_WITH_RIBBON = self::GIFT_HEART;

    const HEART_DECORATION = "\u{1F49F}";
}